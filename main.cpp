...
double cont(double *py,double tint){
 ...
 18, 19, 20, 37, -1};
   ...
   const float pmin(0.001),pint(0.12), dp=pint/(float)nss;
   stringstream fn; 
   ofstream fo;
   int chpar=suc_o;//glu_o;//vfadf;//
...
//   horse.glufl(); return 0;
   double x=solve(&ystart[0],tint);//horse.rotenone(py,tint);// 
//   horse.rotenone(py,tint); cout<<"time="<<tf/double(CLOCKS_PER_SEC)<<endl;
   ofstream fo("nor"); fo<<horse.kkin<<endl; fo.close(); 
//   fo.open("rot"); fo<<horse.kkin<<endl; fo.close(); 
//   int sys=system("gnuplot -e \"fn1='nor'; fno='kin/glu.png'; ax=6.99\" gluplt.p");
    int sys=system("gnuplot -e \"fn1='nor'; fn2='rot'\" xgplt.p");
   horse.nv.write(istor,xi1,0);
//   double xi=cont(&ystart[0],tint); // continuation
//   sys=system("gnuplot gparplt.p");
 //start fitting
