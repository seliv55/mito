...
//        double xi=comb(py);
   double tint(7.);
   double x=horse.rotenone(py,tint);// solve(&ystart[0],tint);//
   ofstream fo("nor"); fo<<horse.kkin<<endl; fo.close(); 
//   horse.rotenone(py); cout<<"time="<<tf/double(CLOCKS_PER_SEC)<<endl;
   fo.open("rot"); fo<<horse.kkin<<endl; fo.close(); 
   int sys=system("gnuplot -e \"fn1='nor'; fn2='rot'; fno='kin/suc.png'; ax=6.99\" gplt.p");
   horse.nv.write(istor,xi1,0);
   double xi=cont(&ystart[0],7); // continuation
