...
if(!ii) fi<<"Time(min)"<<" prod"<<" sp3"<<" fsc2" <<" qsc2" <<" fum"<<" Bypass"<< " qh" << " psi" << " fc1" <<" [Ca²⁺]μM" << " lk"<<" fc2"<<endl;
fi <<tmod<<" " << nv.flx[fc1c]*tfac/60./0.4 <<" "<< sp3 <<" "<< fsc2 <<" " << qsc2 <<" " << conc[nfum] <<" "<< nv.flx[fbp]*tfac/60./0.4 <<" " <<  (conc[nqh]) << " " << conc[npsi] << " " << nv.flx[fc1]*tfac/60./0.4 << " " << conc[nca] <<" "<< nv.flx[flk]*tfac/60./0.4<<" "<<nv.flx[fc2]*tfac/60./0.4<<'\n';
     return conc[npsi];
}
