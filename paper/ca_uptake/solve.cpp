...
int Ldistr::ddisolve(int istart,const double tfin, double *py,ostringstream& fkin) {
  ...
  double t=tex[istart]*tfac,to,rtol=0.0001,atol=1.0e-9,h0=1e-7, hmax=0.25, rpar[2], yprime[NN], rwork[200000];
...
void Ldistr::chast(double *py, double tint) {
  ostringstream fkin,fkont; ros=0.;
  ifin1= ddisolve(0,1,py,fkin); setconc(nca,20.);
