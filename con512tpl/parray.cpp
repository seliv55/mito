...
//parameters:
const int qp_FS(0), rqp_FS(qp_FS+1), FS_c1(rqp_FS+1), rFS_c1(FS_c1+1), qp_bl(rFS_c1+1), rqp_bl(qp_bl+1), bl_bh(rqp_bl+1), rbl_bh(bl_bh+1), bh_qn1(rbl_bh+1), rbh_qn1(bh_qn1+1), bh_qn2(rbh_qn1+1), rbh_qn2(bh_qn2+1), qHbnd(rbh_qn2+1), rqHbnd(qHbnd+1), qnbnd(rqHbnd+1), rqnbnd(qnbnd+1), qpdis(rqnbnd+1), rqpdis(qpdis+1), qhnds(rqpdis+1),rqhnds(qhnds+1), vc1c(rqhnds+1), bypas(vc1c+1), c2ros(bypas+1), vfadf(c2ros+1), vfadr(vfadf+1), vbq1(vfadr+1), vrbq1(vbq1+1), vbq2(vrbq1+1), vrbq2(vbq2+1), vqdis(vrbq2+1), vrqdis(vqdis+1), vqbind(vrqdis+1), vrqbind(vqbind+1), vfred(vrqbind+1), vrfred(vfred+1), vn56(vrfred+1), vrn56(vn56+1), vn2qn1(vrn56+1), vrn2qn1(vn2qn1+1), vn2qn2(vrn2qn1+1), vrn2qn2(vn2qn2+1), vndis(vrn2qn2+1), vrndis(vndis+1), vpbind(vrndis+1), vrpbind(vpbind+1), vros(vrpbind+1), vlk(vros+1), vatsyn(vlk+1), vtca(vatsyn+1), vmdh(vtca+1), vK(vmdh+1), vox(vK+1), vglu_suc(vox+1), vatpase(vglu_suc+1), vglu_oi(vatpase+1), vptp(vglu_oi+1), vgluout(vptp+1), vcs(vgluout+1), vasp_atf(vcs+1), vasp_atr(vasp_atf+1), vGl(vasp_atr+1), vldh(vGl+1), vmalakg(vldh+1), vgluasp(vmalakg+1), vgluOH(vgluasp+1), katase(vgluOH+1), vmdhc(katase+1), rct(vmdhc+1), ntmax(rct+1), ksdh(ntmax+1), kq(ksdh+1), cmal_o(kq+1),  suc_o(cmal_o+1), glu_o(suc_o+1), laco(glu_o+1), k_o(laco+1), Na_o(k_o+1), nNV(Na_o+1);

//concentrations:
const int nqh(0), npsi(nqh+1), nnad(npsi+1), nnadc(nnad+1), npyr(nnadc+1), nsuc(npyr+1), nfum(nsuc+1), nfumc(nfum+1), noaa(nfumc+1), noaac(noaa+1), ncit(noaac+1), nakgm(ncit+1), nakgc(nakgm+1), nglum(nakgc+1), nglu(nglum+1), ngluo(nglu+1), naspm(ngluo+1), naspc(naspm+1), nca(naspc+1), nc2ros(nca+1), nc3ros(nc2ros+1), nk_i(nc3ros+1), npsio(nk_i+1), nNa_i(npsio+1), nAtp(nNa_i+1), neo(nAtp+1), nei(neo+1), nlac(nei+1), nhi(nlac+1), numx(nhi+1);
//fluxes:
