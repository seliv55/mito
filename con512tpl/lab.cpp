...
if(!ii) fi<<"Time(min)"<<" prod"<<" sp3"<<" fsc2" <<" qsc2" <<" fum"<<" Bypass"<< " qh" << " psi" << " fc1" <<" ROS_c2" << " ROS_c3"<<" fc2"<<" [Ca²⁺]μM" <<'\n';
fi <<tmod<<" " << nv.flx[fc1c]*tfac/60./0.4 <<" "<< sp3/c3t <<" "<< fsc2/c2t <<" " << qsc2 <<" " << conc[nfum] <<" "<< nv.flx[fbp]*tfac/60./0.4 <<" " <<  (conc[nqh]) << " " << conc[npsi] << " " << nv.flx[fc1]*tfac/60./0.4 << " " << conc[nc2ros]/c2t <<" "<< conc[nc3ros]/c3t<<" "<<nv.flx[fc2]*tfac/60./0.4<<" "<< conc[nca]<<'\n';
     return conc[npsi];
}
