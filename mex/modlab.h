...
class cI:public Metab{
  int nfmn2,nfmn5,n62,nq;
	public:
  double e6;
  void setnfmn2(int infs,int inq) {nfmn2=infs; nfmn5=8; n62=4; nq=inq;}
...
class cII:public Metab{
  ...
  void read(std::ifstream& fi) {
    for(int i=0;i<len;i++) {fi >> isot[i]; std::cout<<isot[i]<<' ';}
    std::cout<<'\n'; }
...
class Ldistr {
       ...
       static double fmnh[8],fs[8],fmn[8], n5red[8], n6ar[4], n2red[4];
       static double f2s[5],br[5];
       static double fesr[3],c1r[3], bhr[4],sqi[4],qhi[4];
...
float efit(double *py);
	...
	void priout();
       ...
       void distr( double *py,double *pdydt,double);
    ...
    Ldistr():pBC1q(256), qhpBC1(64), BC1qn(64), BC1(15),coreI(31), cIq(96), c1t(0.204), coreII(11), cIIq(36), c2t(0.4), c3t(0.444), qt(7.1), hi(5.4e-05), ho(0.000114), vol(100.), buf(1000000.), fc(500.), frt(0.039), tan(1.), tnad(17.0){}
     ~Ldistr(void) {}
};
}
